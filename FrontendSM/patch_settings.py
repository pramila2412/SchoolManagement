...
file_path = "src/components/SiteSettings.jsx"
...
"""
content = content.replace("export default function SiteSettings() {", defaults_addition + "\nexport default function SiteSettings() {")
...
"""
content = content.replace("const [aboutConfig, setAboutConfig] = useState(ABOUT_DEFAULTS);", "const [aboutConfig, setAboutConfig] = useState(ABOUT_DEFAULTS);" + states_addition)

# 3. Update Fetch Config
fetch_old = """            const [lpRes, aboutRes] = await Promise.all([
                fetch('/api/landing-page'),
                fetch('/api/about')
            ]);"""
fetch_new = """            const [lpRes, aboutRes, curRes, admRes, galRes] = await Promise.all([
                fetch('/api/landing-page'),
                fetch('/api/about'),
...
content = content.replace(fetch_set_old, fetch_set_new)

# 4. Update Save Config
save_old = """            const [resLP, resAbout] = await Promise.all([
                fetch('/api/landing-page', {
                    method: 'PUT',
...
save_new = """            const [resLP, resAbout, resCur, resAdm] = await Promise.all([
...
save_check_old = "if (resLP.ok && resAbout.ok) {"
save_check_new = "if (resLP.ok && resAbout.ok && resCur.ok && resAdm.ok) {"
content = content.replace(save_check_old, save_check_new)
...
"""
content = content.replace("const updateNested = (section, field, value) => {", handlers_addition + "\nconst updateNested = (section, field, value) => {")

# 6. Add Tabs
tabs_old = """                    <button className={`cms-tab ${activeTab === 'about' ? 'active' : ''}`} onClick={() => setActiveTab('about')}>
                        <Info size={16} /> About Page CMS
                    </button>
                </div>"""
tabs_new = """                    <button className={`cms-tab ${activeTab === 'about' ? 'active' : ''}`} onClick={() => setActiveTab('about')}>
                        <Info size={16} /> About Page CMS
                    </button>
                            ...
                            {[0, 1, 2, 3].map(i => (
                                <div key={i} className="form-group">
                                    ...
                                    {curriculumConfig.curriculumImages && curriculumConfig.curriculumImages[i] && (
                                        <img src={curriculumConfig.curriculumImages[i]} alt="Curriculum" className="preview-img" />
                                    )}
...
"""
content = content.replace("                {activeTab === 'about' && (", sections_addition + "\n                {activeTab === 'about' && (")

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("SiteSettings patched successfully.")
